namespace DopLaba1 
{
    public class Facade 
    {
        ...
        */
        ...
        public void TryAddContainer(MyContainer container)
        {
        ...
        public double CountSumMass(List<MyContainer> listOfBox) 
        {
            double sum = 0;
        ...
        */   
        public void GenerateContainers(int k, int lowerBorderMaxCount, int upperBorderMaxCount, double lowerBorderMassBox, double upperBorderMassBox, double lowerBorderPrice, double upperBorderPrice)
        {
            ...
            for (int h = temp; h < k + temp; h++)
            {
                ...
                TryAddContainer(new MyContainer(f, listBoxOfVegetables, h.ToString()));
            }
            }
        }
        public void ChangeMassBox(double mass, BoxOfVegetables boxOfVegetables, MyContainer myContainer) 
        {
            if (store.GetTotalMassContainers() - boxOfVegetables.GetMass() + mass > store.GetMaxStoreMass()) 
            {
                Console.WriteLine("Отказано в замене! Склад не может выдержать такой ящик!");
            }
            else if (store.GetRentalPrice() > CalcMoney(myContainer) - boxOfVegetables.GetMass() * boxOfVegetables.GetPriceForKg() + mass * boxOfVegetables.GetPriceForKg())
            {
        ...
        public void ChangePriceBox(double price,BoxOfVegetables boxOfVegetables, MyContainer myContainer)
        {
            if (store.GetRentalPrice() > CalcMoney(myContainer) - boxOfVegetables.GetMass() * boxOfVegetables.GetPriceForKg() + boxOfVegetables.GetMass() * price)
            {
                throw new Exception("Невозможно изменить цену! Нерентабельное хранение");
            }
            else 
            {
                boxOfVegetables.SetMass(price);
            }
        }
        }
